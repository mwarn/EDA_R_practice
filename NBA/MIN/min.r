...
ggplot(aes(x = CS_FG_PCT, y = PLAYER), data = data_min, stat = 'identity') +
  ...
  theme(axis.text.x = element_text(colour = '#0a3495', size = 14),
        ...
        panel.background = element_rect(fill = '#44c430')) +
  xlab('Catch & Shoot Field Goal Percentage') +
  ylab('') +
  labs(size = 'C&S Attempts/Game')

ggplot(aes(x = CS_FG3_PCT, y = PLAYER), stat = 'identity', data = data_min) +
...
ggplot(aes(x = PU_FG_PCT, y = PLAYER), data = data_min, stat = 'identity') +
...
ggplot(aes(x = PU_FG3_PCT, y = PLAYER), stat = 'identity',  data = data_min) +
...
ggplot(aes(x = PASS_PG, y = PLAYER), stat = 'identity', data = data_min) +
...
ggplot(aes(x = DVS, y = PLAYER), data = data_min, stat = 'identity') +
...
ggplot(aes(x = DIST_OFF/GP, y = PLAYER), data = data_min) +
...
ggplot(aes(x = DIST_DEF/GP, y = PLAYER), data = data_min, stat = 'identity') +
  geom_point(aes(size = factor(MIN)), color = '#331163') +
  theme(axis.text.x = element_text(colour = '#0a3495', size = 14),
        axis.text.y = element_text(colour = '#0a3495', size = 14),
        panel.background = element_rect(fill = '#44c430')) +
...
ggplot(aes(x = FGA_CLOSE, y = PLAYER), data = data_min, stat = 'identity') +
